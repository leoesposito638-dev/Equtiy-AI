// ============================================================================
// Equity AI — Milestone 6B, Batch 2: AMZN, TSLA, NVO, SPOT
//
// Runs the EXACT same, already-verified pipeline used for NVDA and Batch 1
// (Milestones 1-4A, 6B Batch 1) for four more real, already-seeded companies:
...
//
// No pipeline code is modified by this script. Does not create companies
// (getCompanyIdByTicker only looks up existing rows). Does not touch
// metric_benchmarks or generate any score. Each company is wrapped in its
// own try/catch so an external FMP entitlement failure (as ASML hit in
// Batch 1) or any other single-company error stops that company honestly
// and does not prevent the remaining companies in the batch from running.
//
// Run with:
//   npm run ingest:batch2
// ============================================================================
...
const BATCH_2_TICKERS = ["AMZN", "TSLA", "NVO", "SPOT"];
...
async function main() {
  console.log(`Equity AI — Milestone 6B Batch 2: ${BATCH_2_TICKERS.join(", ")}\n`);
  ...
  for (const ticker of BATCH_2_TICKERS) {
    console.log(`\n${"=".repeat(70)}\n${ticker}\n${"=".repeat(70)}`);
    try {
  ...
  console.log(`\n${"=".repeat(70)}\nBatch 2 complete. STOPPING — Batch 3 not started.\n${"=".repeat(70)}\n`);
}
