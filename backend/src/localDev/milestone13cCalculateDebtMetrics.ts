// ============================================================================
// Equity AI — Milestone 13C Phase B/C/D: compute and store total_debt,
// net_debt, ebitda, debt_to_equity, net_debt_to_ebitda for all 30 demo
// companies via the real, extended calculateAndStoreFundamentalRatios()
// (unmodified driver signature except the new optional ticker param used to
// gate Net Debt's cash-cleanliness — see supabaseFundamentalRatiosRepo.ts).
// Existing PROFITABILITY/FINANCIAL_HEALTH/COMPETITIVE_ADVANTAGE ratios from
// 12B are recomputed too but will gracefully skip as already_exists.
//
// Run with:
//   npx ts-node --transpile-only src/localDev/milestone13cCalculateDebtMetrics.ts
// ============================================================================
...
const NEW_METRICS = ["total_debt", "net_debt", "ebitda", "debt_to_equity", "net_debt_to_ebitda"];

async function main() {
  console.log(`Equity AI — Milestone 13C: calculate debt-derived metrics (30-company demo universe)\n`);
  ...
  for (const ticker of TICKERS) {
    ...
    console.log(`${ticker.padEnd(6)} debt-metric rows: ${newMetricOutcomes.length} (${stored.length} new) — ${[...byMetric.entries()].map(([m, n]) => `${m}=${n}`).join(", ") || "none"}`);
