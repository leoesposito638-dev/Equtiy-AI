// ============================================================================
// Equity AI — Milestone 12D Phase 9/10: real write. Runs the unmodified
// calculateFundamentalScore() through the real, Milestone-12C-corrected
// storeFundamentalScore() for all 30 demo companies — this both (a) persists
// PROFITABILITY's new margin_trend-informed score/confidence/coverage and
// (b) recomputes and persists the Fundamental Score reflecting it, via the
// same idempotent upsert path verified in Milestone 12C.
// ============================================================================
...
async function main() {
  ...
  console.log(`Equity AI — Milestone 12D: real Fundamental Score + category_scores write (30-company demo universe)\n`);
  ...
  for (const c of companies! as any[]) {
    ...
    console.log(`${c.ticker.padEnd(6)} score=${result.score.toString().padStart(6)} confidence=${result.confidence.toString().padStart(6)} coverage=${result.dataCoverage.toString().padStart(6)} categories=${contributing.length}/8 (${contributing.join(",")})`);
  }
